...
export function localCoachReply(input, bizA, bizB, tasks, dels) {
  ...
  if (t.includes('delegate')) {
    const cands = tasks.filter(t => !t.done && t.quadrant === 'urgent-notimportant').slice(0, 3);
    return cands.length
      ? `Delegate these:\n${cands.map(t => '- ' + t.text).join('\n')}\nOwner + deadline + clear outcome for each.`
      : 'Nothing obvious. Look for admin, chasing, or anything a team member could do at 80%.';
  }
  return `You're running ${bizA}, ${bizB}, and real life. One thing that matters, one quick win, one thing to stop carrying alone.${dels.length ? ` ${dels.length} item${dels.length > 1 ? 's' : ''} delegated.` : ''}`;
}

export function checkinReply(score, done) {
  if (score <= 2) return `Rough one. Still — ${done} thing${done === 1 ? '' : 's'} done means the day wasn't wasted. Tomorrow: one clear win before anything reactive.`;
  if (score === 3) return `Solid. Not spectacular. That's fine. ${done} thing${done === 1 ? '' : 's'} moved. Tomorrow, protect your first hour.`;
  return `Good day. ${done} done. Tomorrow: repeat what worked.`;
}

export const QUADS = [
  { id: 'urgent-important',       label: 'Do first',  color: '#C94B2A' },
  { id: 'noturgent-important',    label: 'Schedule',  color: '#10B981' },
  { id: 'urgent-notimportant',    label: 'Delegate',  color: '#F59E0B' },
  { id: 'noturgent-notimportant', label: 'Eliminate', color: '#5A5A68' },
];

export const QUAD_ORDER = {
  'urgent-important': 0,
  'noturgent-important': 1,
