    ...
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
            ...
            {[...bt].sort((a, b) => QUAD_ORDER[a.quadrant] - QUAD_ORDER[b.quadrant])
              .map(t => (
                <TaskRow key={t.id} task={t} now={now} bizA={bizA} bizB={bizB}
                  ...
                  onReclassify={onReclassify} onFocus={onFocus} />
              ))}
          </div>
        );
      })}
      ...
      {done.length > 0 && (
        <div style={{ ...S.card, opacity: 0.5 }}>
          <span style={S.lbl}>Completed ({done.length})</span>
          {done.slice(0, 8).map(t => (
            <div key={t.id} style={S.row}>
              <input type="checkbox" checked onChange={() => onToggle(t.id)}
                style={{ accentColor: C.green, width: 15, height: 15, cursor: 'pointer' }} />
              <span style={{ flex: 1, fontSize: 13, color: C.textDim,
                textDecoration: 'line-through' }}>{t.text}</span>
